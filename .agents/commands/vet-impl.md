...
- Use the `verify-impl` Agent Skill to drive every phase: build project context, extract every verifiable requirement, discover the implementation surface, classify each requirement (PASS / DRIFT / PARTIAL / MISSING / CONFLICT) with evidence, run the cross-cutting and self-verification passes, then emit the verdict and remediation plan.
...
- Load the `verify-impl` Agent Skill before reading the specification.
