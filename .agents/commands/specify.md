...
2. **Specifies** - delegates to the architect agent, which loads the `writing-specs` skill and grounds the spec in project context; the spec clears the skill's structural validator before it may proceed to review, and the validator's size warnings are carried forward as measurements rather than as blockers
...
4. **Revises if needed** - severity-gated: Significant-only findings trigger one revision; Critical findings enter a Critical Resolution Loop with up to two re-review cycles, then halt if unresolved; every revision re-passes the validator gate
