...
5. **Orphaned references.** If the spec cites an architecture section (e.g. `Section X.Y of docs/architecture.md#xy-...`), an ADR (`ADR-0014`), or a tracker reference, the target MUST exist. Verify every citation before delivering. Broken citations indicate the spec drifted from its sources.
...
- [ ] Risk-assessment table has at least one data row. Prefer eight rows or fewer; the validator reports a count above that as a warning, not a failure. When over budget, drop the rows that assert an absence of change and merge those that name the same observable failure. Severity is one of the values the project documents (typically: Critical, High, Medium, Low; or Critical, Major, Minor). Every Critical and High row has a concrete mitigation, not "TBD".
...
- [ ] Open-questions section prefers five questions or fewer, each blocking a design decision; a higher count is a warning, not a failure. When over budget, move the questions that carry a recommendation into the technical-architecture section as decisions. If the spec genuinely has no open questions, state that explicitly; an empty section is suspicious.
