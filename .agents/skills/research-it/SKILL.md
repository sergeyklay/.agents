...
- `isolate-cli` skill - for questions the installed binary, the shipped bundle or on-disk configuration answers: which paths a tool searches, which config file wins, what a key decodes to
...
These are not anti-patterns of writing (those live in the `explain-it` skill). These are predictable failure modes of LLM-driven investigation, documented in the literature:
