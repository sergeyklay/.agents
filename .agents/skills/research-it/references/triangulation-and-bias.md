...
- **Baseline corpus contaminated by the subject.** A rate is read off a corpus chosen as a control, such as human-written code a lint rule has to stay silent on, while the control carries copies of the subject itself. A dependency cache is the usual carrier: `$GOMODCACHE` holds every released version of any module the host has built, so a rule aimed at one AI-written Go repository returned 263 hits on the "human" module cache, 261 of them in that repository's own published modules. The scope resolves, a positive control passes and the count reads as a false-positive rate, so every defence above stays silent. Defence: rank the control's hits by the path that contributes them before reading any rate off it, exclude every path that belongs to the subject (`rg -g '!**/<owner>/**'`), and read a sample of what remains in situ.
