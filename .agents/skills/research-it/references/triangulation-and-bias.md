...
Modes expanded below point to their section. The rest give the symptom, then the mitigation.

- **Hallucination of citations.** See § Hallucinated citations below.
- **Confidence miscalibration.** See § Confidence miscalibration below.
- **SEO content-farm preference.** See § SEO content-farm preference below.
- **Anchoring on the first plausible source.** See § Anchoring on the first source below.
...
- **Snippet summarisation.** See § Snippet summarisation below.
- **Summariser-bounded negatives.** See § Summariser-bounded negatives below.
...
- **Silent-zero search results.** See § Silent-zero search results below.
...
- **Tool-reclassified categories.** See § Tool-reclassified categories below.
...
- **Baseline corpus contaminated by the subject.** A rate is read off a corpus chosen as a control, one the rule, metric or check under study is supposed to stay silent on, while the control carries copies of the subject itself. The usual carriers are a dependency cache, a vendored directory, a fork or mirror, and an archive of past releases. A rule aimed at one repository returned 263 hits on a dependency cache taken as the independent baseline, and 261 of them sat in that repository's own published releases. The scope resolves, a positive control passes and the count reads as a false-positive rate, so every defence above stays silent. Defence: before reading any rate off a control, rank its hits by the path or origin that contributes them, exclude everything that belongs to the subject, and read a sample of what remains in situ.
...
OpenAI reported this limitation for Deep Research in February 2025: the system showed weakness in confidence calibration and often failed to convey uncertainty accurately ([source](https://openai.com/index/introducing-deep-research/)).
...
Anthropic reported in 2025 that its early research agents consistently chose SEO-optimised content farms over less highly ranked authoritative sources; source-quality heuristics mitigated the problem ([source](https://www.anthropic.com/engineering/multi-agent-research-system)).
...
**Defence.** After the first source, deliberately search using **different vocabulary** - synonyms, the opposing-camp terminology, the academic term if the first source was a blog post (or vice versa). Consult at least one source from a different tier. The second search should not look like a refinement of the first.
...
2. **Run a positive control.** Query the same scope for a term that must exist (the project's own name, `README`, a known top-level symbol). Zero hits on the control means the instrument cannot see the scope, and every zero it produced is void. Switch instruments: fetch the tree or release tarball at the relevant tag, grep it locally, and rerun the control there.
