...
**No number assigned** (standalone use) `scripts/next_adr_number.sh` to get the next available number:

```bash
# Output: 0004
```

For batch allocation (multiple ADRs in one session) `scripts/next_adr_number.sh --count 3` to reserve a block of numbers:
