...
Ten modes, and a floor above zero reaches four of them. Three of the middle four each need one more clause rather than a new mechanism. The fuzz row needs a clause plus the run's mode, and only the caller can supply that. The last two are the limit: read on.
