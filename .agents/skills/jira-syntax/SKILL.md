...
```bash
python3 scripts/validate_jira_syntax.py <file>
```

The script checks for Markdown syntax used where Jira markup is expected, unclosed block macros, headings missing the space after the period, and code blocks without a language identifier. It accepts multiple files and supports `--no-color` for non-TTY output.
