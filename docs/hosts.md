...
The translation is necessarily narrower than Claude in some places. Codex role files can disable the shell, app, and plugin surfaces and can hide skills, but they cannot independently restrict `apply_patch`, web search, delegation, or inherited MCP servers. Skill allow-lists expose the selected skills rather than eagerly loading them. Sandbox and approval settings are inherited because the role application path does not enforce role-local values. Disabling the shell also removes shell-backed file search, so restricted roles rely on the remaining hosted tools and delegation.
