...
Delegate the entire pipeline to the `conductor` subagent via the `Agent` tool with `subagent_type: conductor`. `conductor` is a subagent, not a skill; never pass it to the `Skill` tool.
