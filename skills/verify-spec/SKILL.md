...
name: verify-spec
description: >
  Forensic verification of implementation code against its authoritative
  specification. Use when checking whether code faithfully implements every
  requirement from a spec, verifying spec conformance after implementation,
  auditing compliance between design and code, or when asked to 'verify this
  implementation matches the spec', 'check spec conformance', 'does the code
  match the design', 'audit implementation against requirements', or 'verify
  spec coverage'. Extracts every verifiable requirement from the spec, maps
