 ...
 *
 * A read-only, fully-discoverable Model Context Protocol server for local
...
//
// __dirname is .github/mcps/ - two levels up is the project root.
...
// ── Process lifecycle - zombie-killer ─────────────────────────────────────────
//
// Claude Code and Cursor do not always kill MCP child processes on exit. Two
// independent watchdogs ensure we self-terminate:
//
//   ppid watcher - polls ppid; fires when the parent process is replaced
//   stdin HUP    - Node's 'end' event fires when the host closes its pipe end
//
// Reference: https://forum.cursor.com/t/exiting-the-cursor-will-not-kill-the-mcp-process/74296
...
if (os.platform() !== "win32") {
  const ppidTimer = setInterval(() => {
    if (process.ppid !== INITIAL_PPID) {
      logInfo("Parent process exited - shutting down.");
      process.kill(process.pid, "SIGTERM");
    }
  }, 2_000);
  ppidTimer.unref(); // Must not prevent Node event-loop exit independently

  // Do NOT call process.stdin.resume() here - StdioServerTransport owns stdin
  // and will set flowing mode itself. Calling resume() before the readline
  // interface attaches causes the initialize message to be silently discarded.
...
async function fetchCatalog(): Promise<Catalog> {
  ...
  const catalog = await withDb(async (client) => {
    ...
    for (const r of tableColRows) {
      ...
      const col: CatalogColumn = { name: r.column_name, type: r.column_type };
...
const tableTemplate = new ResourceTemplate(
  "postgres://table/{schema}/{table}",
  {
    list: async () => {
      ...
      const hit =
        getCached<Array<{ uri: string; name: string; mimeType: string }>>(
          cacheKey,
        );
...
server.registerResource(
  ...
  async (uri: URL, variables: Record<string, string | string[]>) => {
    ...
    try {
      ...
      return {
        contents: [
          { uri: uri.href, mimeType: "text/plain", text: renderTableDDL(def) },
        ],
      };
    } catch (err) {
      return {
...
server.registerTool(
  "query",
  {
    ...
    inputSchema: {
      sql: z.string().min(1).describe("Read-only SQL statement to execute."),
      limit: z
        .number()
        .int()
        .min(1)
  ...
  async ({ sql, limit }) => {
    ...
    if (validationError) {
      return {
        content: [
          { type: "text", text: `Validation error: ${validationError}` },
        ],
        isError: true,
      };
    }
...
server.registerTool(
  ...
  async ({ sql, analyze }) => {
    ...
    try {
      ...
      return {
        content: [{ type: "text", text: `Explain error: ${String(err)}` }],
        isError: true,
      };
    }
  },
...
async function main(): Promise<void> {
  ...
  await withDb(async (client) => {
    ...
    logInfo(`Connected: database="${rows[0]?.db}", PostgreSQL ${rows[0]?.ver}`);
  });
